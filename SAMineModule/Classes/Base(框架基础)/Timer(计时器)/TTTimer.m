...
- (void)setPrecision:(NSInteger)precision{
    ...
    self.onceTime = [self.class value: (NSTimeInterval)precision byOpration: OMDecimalOprationTypeDivide percision: 6 withValue: 1000.0].doubleValue;
}
...
- (void)reCalculateMinder{
    NSTimeInterval blank = [self.class value: self.appDidEnterForegroundTime byOpration: OMDecimalOprationTypeSubtract percision: 6 withValue: self.appDidEnterBackgroundTime].doubleValue;
    if (self.isAscend) {
        NSTimeInterval theNewValue = [self.class value: self.currentTime byOpration: OMDecimalOprationTypeAdd percision: 6 withValue: blank].doubleValue;
        if (theNewValue > self.endTime) {
            if (self.progressBlock) {
                TTTime *progress = [self timeProgress: theNewValue];
        ...
        NSTimeInterval theNewValue = [self.class value: self.currentTime byOpration: OMDecimalOprationTypeSubtract percision: 6 withValue: blank].doubleValue;
        if (theNewValue <= 0) {
            if (self.progressBlock) {
...
/** 计算时间进度*/
- (TTTime *)timeProgress: (NSTimeInterval)time{
    ...
    TTTime *progress = [[TTTime alloc] init];
...
- (void)configure{
    ...
    [self initTimerCompletion:^{
        ...
        TTTime *progress = [self timeProgress: self.currentTime];
        if (self.isAscend) {
            self.currentTime = [self.class value: self.currentTime byOpration: OMDecimalOprationTypeAdd percision:6 withValue: self.onceTime].doubleValue;
            ...
            self.currentTime = [self.class value: self.currentTime byOpration: OMDecimalOprationTypeSubtract percision: 6 withValue: self.onceTime].doubleValue;
