...
@implementation TTAnyCornerRadius
...
+(void)cornerRadiusWithView:(UIView *)view radiiMake:(CornerRadii)cm
{
    ...
    CGPathRef path = CYPathCreateWithRoundedRect(view.bounds,cm);
...
{
    //切圆角
    if (view.layer.ttAnyBorderLayer) {
        [view.layer.ttAnyBorderLayer removeFromSuperlayer];
    }
    CAShapeLayer *shapeLayer = [CAShapeLayer layer];
    CGPathRef path = CYPathCreateWithRoundedRect(view.bounds,cm);
    ...
    view.layer.ttAnyBorderLayer = shapeLayer;
}


@end

@implementation CALayer (TTAnyCornerRadiusBorderLayer)
/**
  设置当前controller 在 segmentController 中的下标
*/
const char ttAnyBorderLayerKey;

//set
-(void)setTtAnyBorderLayer:(CALayer *)ttAnyBorderLayer{
    objc_setAssociatedObject(self, &ttAnyBorderLayerKey, ttAnyBorderLayer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
//get
- (CALayer *)ttAnyBorderLayer{
    return objc_getAssociatedObject(self, &ttAnyBorderLayerKey);
}
